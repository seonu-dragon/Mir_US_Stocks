...
- **기업개요 · 목표주가 범위 (2026-09-26, `company-info.js` + `company-info-core.js`)**: 목표주가 범위는 종목 상세 좌측 요약 패널, 기업개요는 개요 탭.
  ...
  - 목표주가 범위(`#priceTargetCard`, US 만): Nasdaq 컨센서스 최저—평균—최고 바 위에 현재가 점, "평균 목표가는 현재가보다 N% 높습니다/낮습니다" 서술만(권유 문구 없음), 매수·보유·매도 분포 막대와 참여 애널리스트 수, 접힌 "월별 의견 분포"(약 12개월). Nasdaq 월별 이력의 값(y)은 목표가가 아니라 그달 주가라 목표가 이력은 없다. KR 은 FnGuide 컨센서스에 최고·최저가가 없어 기존 컨센서스 카드만 둔다.
