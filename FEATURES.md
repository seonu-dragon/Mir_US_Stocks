...
- **종목 상세 구성 (UI 2단계, 2026-09-26, `stock-view.js`)**: 종목 › 분석은 **좌측 고정 요약 패널 + 본문 6탭**이다. 좌측(PC 336px, 스크롤해도 따라옴 · 1100px 이하에서는 본문 위 1단): 종목명·시장·업종, 가격(28/700)·등락 `▲5.04 (+1.50%)`·기준 시각, 관심 ☆, 짧은 지표, 가설 추가·적립식·링크 복사, 시세정보(52주 게이지), 투자정보(PER 기준 라벨), KR 애널리스트 컨센서스, US 목표주가 범위, 데이터 품질(접힘). 모바일은 시세정보까지 보이고 나머지는 '투자정보 더보기'. 본문 탭 — **개요**: 차트·기술 점수 분석·AI 진단 한 줄·왜 상승했나·투자 체크리스트·주요 뉴스·산업 지표·기업개요 / **재무**: 핵심 지표·재무·실적 일정/EPS 기록/발표 반응·실적 추정치 변화 / **밸류**: 역DCF·시나리오 DCF·PER/PBR/PSR 밴드·업종 상대 팩터 등급 / **이벤트·공시**: 종목 이벤트(옵션 만기·목표가·배당·뉴스·가격 이벤트·커뮤니티)·과거 이벤트 반응 / **수급·보유**: KR 수급(5일·20일·일별 보기)·기업집단·국민연금, US 스마트머니 종합·정치인 매매, ETF 구성 종목 / **AI 진단**: AI 투자 분석 진단. 탭은 `&view=overview|fin|val|events|flow|ai` 로 URL 에 남고(탭을 누르면 갱신), `&dcf=` 공유 링크는 밸류 탭을 연다. 숨은 탭은 화면 밖 배치라 숨긴 채 그려도 SVG 폭이 맞다. 자동 AI 리포트는 AI 진단 탭을 처음 열 때(또는 캐시가 있을 때)만 부르고, US 수급·보유 탭의 무거운 데이터(내부자·의회·13F)도 그 탭을 열 때 받는다. 스모크 `scripts/smoke_ui.py` 의 `test_stock_views`.
- **인터랙티브 종목 차트**:
...
- **기업개요 · 목표주가 범위 (2026-09-26, `company-info.js` + `company-info-core.js`)**: 목표주가 범위는 종목 상세 좌측 요약 패널, 기업개요는 개요 탭.
  ...
  - 목표주가 범위(`#priceTargetCard`, US 만): Nasdaq 컨센서스 최저—평균—최고 바 위에 현재가 점, "평균 목표가는 현재가보다 N% 높습니다/낮습니다" 서술만(권유 문구 없음), 매수·보유·매도 분포 막대와 참여 애널리스트 수, 접힌 "월별 의견 분포"(약 12개월). Nasdaq 월별 이력의 값(y)은 목표가가 아니라 그달 주가라 목표가 이력은 없다. KR 은 FnGuide 컨센서스에 최고·최저가가 없어 기존 컨센서스 카드만 둔다. 점 티커(BRK.B·BF.B·HEI.A 등 주식 클래스)는 Nasdaq 이 심볼은 알아보지만 targetprice 가 비어 있어(BRK-B·BRK/B·BRK^B 는 400/404, 2026-09-26 실측) Yahoo quoteSummary(`financialData` 목표가 + `recommendationTrend` 이번 달 의견, 심볼 BRK-B)로 보충하고 레코드 `src:"yahoo"` 로 출처를 Yahoo Finance 로 표기한다(월별 이력 없음, 저장 키는 원래 티커). 다른 클래스 목표가를 옮겨 쓰지 않는다. 점 티커는 시총 상위 1,000 밖이어도 대상에 넣는다. LEN.B 처럼 양쪽 다 없으면 카드를 숨긴다.
