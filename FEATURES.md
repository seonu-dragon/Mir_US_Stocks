...
- **시장별 등락색 · 디자인 토큰(UI 1단계, 2026-09-26)**: 한국 모드는 상승 빨강·하락 파랑, 미국 모드는 상승 초록·하락 빨강. `<html data-market>` 에 따라 `--pos/--neg` 토큰이 바뀌고 캔들·트리맵·스파크라인·수급 표·AI 모드 캔버스가 이를 따른다(JS 는 `var(--pos)` 또는 `mirColor()`). '좋음/나쁨' 의미색은 `--good/--bad` 로 분리. 글꼴 7단계(12·13·14·16·18·22·28), 굵기 500/600/700, 구분선 rgba 한 가지, 카드 그림자 없음, 흰 헤더·흰 차트 캔버스(다크는 중립 흑회색), 표는 행 구분선만, 칩·탭 선택은 검정(ink). 큰 히어로 검색은 오늘 탭에만 보이고 다른 탭은 헤더 검색(Ctrl K)을 쓴다.
