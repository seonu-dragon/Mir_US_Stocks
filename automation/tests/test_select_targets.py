...
def test_select_overseas_fallback_when_mentions_overlap():
    scanner = [{"ticker": f"S{i}", "probability_score": 80 - i} for i in range(20)]
    mentions = [{"ticker": f"S{i}", "mention_count": 100 - i} for i in range(8)]
    selected = select_overseas_targets(scanner, mentions, total=15)
    assert len(selected) == 15
    assert len({item["ticker"] for item in selected}) == 15