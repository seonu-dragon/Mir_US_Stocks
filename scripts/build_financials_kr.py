...
호출 예산(DART 개인 키 하루 한도 — 같은 주간 워크플로우의 소유구조·감사의견 빌더와 나눠 쓴다)
  ...
  ③ 3년 전 사업보고서 → ④ 이전 연도 주식수 → ⑤ 6년 전 사업보고서. 최신 연도 주식수는 ①에 든다(역DCF 필수). 받은 보고서는 종목 파일의 `_raw.reports` 에
...
def tiers(today: date) -> list[list[tuple[str, int, str]]]:
    ...
    # 최신 사업보고서 주식수도 1계층에(종목당 1회). 2026-09-26: 주식수가 4계층이라 940종목 중 910종목이
    # 비어 있었고, 역DCF 가 463종목에서 '주식수 공시가 없어 계산하지 않습니다' 로 멈췄다.
    t1.append(("S", latest_a, "11011"))
    t2 = [("F",) + r for r in recent8 if ("F",) + r not in t1]
    t3 = [("F", latest_a - 3, "11011")]
    t4 = [("S", latest_a - i, "11011") for i in range(1, 5)]
