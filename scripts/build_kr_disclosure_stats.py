...
- 날짜로 묶어서 검정한다(by_day). 공시는 제출기한에 뭉치므로 이벤트를 독립 표본으로
  세면 t 가 부풀려진다.
- 신호 판정은 D+1 로만 한다. D0 는 공시 당일이라 예측이 아니고, 접수 시각이 없어서
  그 움직임이 공시 전인지 후인지도 못 가린다.

## 결론 (5년 · 공시 554,483건 · 반응 141,102건 · 유형 41개)

거래 가능한 우위는 없다. edge=true 는 0개다.

이벤트 단위로는 3개가 무작위를 이기는 것처럼 보였지만, 날짜로 묶자 둘이 사라졌다:

    지속가능경영보고서  D+1  t +5.25 → -0.09   (712건 중 115건이 2026-06-30 하루)
    계열사 금융거래     D+1  t -4.13 → -0.94

남은 하나(증자·사채 D0)는 날짜로 묶어도(t +4.21), 날짜별 중앙값으로 봐도(t +4.77)
버틴다. 실재하는 효과지만 D0 다 — 공시 당일이라 행동할 수 없다. 거래 가능한 D+1 은
t +0.60 으로 무작위와 구분되지 않는다. 게다가 D0 이전 5일에 이미 +1.76% 올라 있어
'주가가 오른 뒤 증자를 낸다' 는 역인과로 보인다. 그래서 sameDayOnly 로만 표시한다.

지지/저항 때와 같은 결론이다. 공시 유형은 시각 보조일 뿐 예측 지표가 아니다.
...
def reaction(closes: list[float], i: int, mkt: list[float], mi: int,
             date: str | None = None) -> dict | None:
    """i 일 기준 반응(초과수익). 앞뒤 여유가 없으면 None.

    date 는 by_day 가 같은 날 이벤트를 묶는 데 쓴다.
    """
...
def by_day(rows: list[dict], key: str) -> list[float]:
    """같은 날짜의 이벤트를 하나로 묶어 날짜별 평균을 낸다.

    공시는 날짜에 뭉친다. 지속가능경영보고서 712건 중 115건이 2026-06-30 하루에
    몰려 있다(6월말 제출기한). 그 115종목의 D+1 수익률은 독립된 115개 관측이 아니라
    '그날 하루' 하나다 — 그날 시장이 좋았으면 115건이 한꺼번에 플러스가 된다.

    Welch t 가 n=712 를 독립 표본으로 믿으면 t 가 부풀려진다. 실측: 이벤트 단위
    t=+5.25 로 '무작위를 이긴다' 던 것이 날짜로 묶으면 t=-0.09 로 사라졌다.
    계열사 금융거래도 -4.13 → -0.94 로 사라진다. 둘 다 착시였다.
    """
    buckets: dict[str, list[float]] = {}
    for r in rows:
        d = r.get("date")
        if d:
            buckets.setdefault(d, []).append(r[key])
...
def build(limit_types: int | None):
    ...
    for e in events:
        ...
        if mi is None:
            continue
        r = reaction(closes, i, mcloses, mi, dates[i])
        if not r:
            continue
        ...
        for _ in range(RANDOM_PER_EVENT):
            ...
            if mj is None:
                continue
            rr = reaction(closes, j, mcloses, mj, dates[j])
            if rr:
    ...
    for typ, rows in by_type.items():
        ...
        for k in ("d0", "d1"):
            ...
            td = welch_t(by_day(rows, k), by_day(rnd, k)) if rnd else None
            if t is not None:
                rec["t"][k] = round(t, 2)
            if td is not None:
                rec["tDay"][k] = round(td, 2)
            # 날짜로 묶은 t 를 기준으로 판정한다(아래 by_day 주석 참고).
            if (td is not None and abs(td) >= 3 and t is not None and abs(t) >= 3
                    and abs(rec[k]["mean"] - (rec[k].get("random") or {}).get("mean", 0)) >= 0.5):
                edge = True
        # D0 는 공시 당일이라 예측이 아니다. DART list.json 은 날짜만 주고 접수 시각이
        # 없어서, 그 움직임이 공시 전인지 후인지조차 못 가른다(증자·사채는 D0 이전
        # 5일에 이미 +1.76% 올라 있다 — 주가가 오른 뒤 증자를 내는 역인과로 보인다).
        # 실제로 행동할 수 있는 시점은 D+1 뿐이라, 신호 여부는 D+1 로만 판정한다.
        rec["edge"] = edge and "d1" in rec["tDay"] and abs(rec["tDay"].get("d1", 0)) >= 3
        rec["sameDayOnly"] = edge and not rec["edge"]
        stats[typ] = rec
    if limit_types:
